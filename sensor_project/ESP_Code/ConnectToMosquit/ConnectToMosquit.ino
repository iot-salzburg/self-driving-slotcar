...
void read_format_data() {
  // get mpu data and put it into the data_buffer, ready to send.
  Wire.beginTransmission(i2c_addr);
  ...
  data_index += sprintf((data_buffer+data_index), "{\"AcX\": %d, \"AcY\": %d, \"AcZ\": %d, \"GyroX\": %d, \"GyroY\": %d, \"GyroZ\": %d, \"Time\": %d},", AcX, AcY, AcZ, gyroX, gyroY, gyroZ, (now_time - begin_time));
}
